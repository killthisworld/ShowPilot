            ...
            <div className="grid grid-cols-7 gap-1">
              {daysInMonth.map((day) => {
                ...
                return (
                  <div
                    key={key}
                    className={`rounded-xl overflow-hidden transition-colors ${isCurrentMonth ? "bg-[#161616]" : "bg-transparent"} ${isToday ? "ring-1 ring-[#8CFF3D]/50" : ""} ${hasShows && isCurrentMonth ? "cursor-pointer hover:bg-[#1f1f1f]" : ""}`}
                  ...
                  >
                    ...
                    {dayShows.length > 2 && (
                      <button onClick={() => openShow(dayShows[2].id)} className="w-full px-1 pb-1">
                        <span className="text-[8px] text-white/30">+{dayShows.length - 2} more</span>
                      </button>
                    )}
                  </div>
                );
