...
import { Home, CalendarDays, PlaneTakeoff, Settings } from "lucide-react";

const TABS = [
  { path: "/band", label: "Home", icon: Home },
  { path: "/band/calendar", label: "Calendar", icon: CalendarDays },
  { path: "/band/cockpit", label: "Cockpit", icon: PlaneTakeoff },
  { path: "/band/settings", label: "Settings", icon: Settings },
];
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#0d0d0d]/95 backdrop-blur-lg border-t border-[#1e1e1e] pb-3">
      <div className="flex items-center justify-around max-w-lg mx-auto h-16 px-4">
        {TABS.map((tab) => {
          const active = tab.path === "/band" ? pathname === "/band" : pathname.startsWith(tab.path);
          return (
            <Link
              key={tab.path}
              to={tab.path}
